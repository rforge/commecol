compas<-function(S, dims, Am, beta, coords, n.quali, add1){

n<-nrow(coords)
if(ncol(coords)==1){coords<-cbind(coords, rep(0,n))}

alfa<-matrix(runif(dims*S,0.1,5), S, dims)		
gama<-matrix(runif(dims*S,0.1,5), S, dims)
A<-rlnorm(S, Am) 			                  #Abundance of each species in the modal point
m<-matrix(runif(dims*S,-50,150), S, dims)	#Position of the modal point in the gradient
r<-matrix(, S, dims)

for(i in 1:dims){
   r[,i]<-rnorm(S, mean=100*beta[i], sd=0.3*100*beta[i])}#Range size of S spp.(rows) in dims gradients(cols)

b<-alfa/(alfa+gama)
d.temp<-matrix(, S, dims)
...
###Quantitative Noise
###A given value is replaced by a random value from a Poisson distribution with lambda= given value
for(k in 1:n){
   for(i in 1:S){
      resu[k,i]<-rpois(1,resu[k,i])}}

###Qualitative Noise; sp has probability '1-n.quali' of occurring in a site within its range. The command replaces n.quali*100% of the values by "0".
for(i in 1:S){
...
###Response Curves without noise for 1 gradient simulations.
###The function does not create plot for dims>1.
if(dims==1){
  plot(x=0, y=0,xlab="Gradient",ylab="Abundance",xlim=c(0,100),ylim=c(0,max(A)),
        main="Response curves WITHOUT quali- and quantitative noises",type="n") 
  for(i in 1:S){
